...
<?php
  session_start();
  include(''); // Fichier PHP contenant la connexion à votre BDD  
  ...
  $req = $BDD->query("SELECT t.*, DATE_FORMAT(t.date_creation, 'Le %d/%m/%Y à %H\h%i') as date_c, U.prenom
    FROM topic T
...
<?php
  ...
  $req = $DD->query("SELECT t.*, DATE_FORMAT(t.date_creation, 'Le %d/%m/%Y à %H\h%i') as date_c, U.prenom
    FROM topic T
    LEFT JOIN utilisateur U ON U.id = T.id_user
    WHERE t.id = ? AND t.id_forum = ?
    ORDER BY t.date_creation DESC", 
  ...
  $req_commentaire = $BDD->query("SELECT TC.*, DATE_FORMAT(TC.date_creation, 'Le %d/%m/%Y à %H\h%i') as date_c, U.prenom, U.nom
...
<html>
  ...
  <body>
    <?php
      require_once('../menu.php'); //refaire le chemin   
      ?>
      <div class="container">
        <div class="row">   
